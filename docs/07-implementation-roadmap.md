...
基盤の `AIService`、Mock、PHP proxy、feature flagを先に準備し、UIが実装された時点で段階的に接続する。#14/#19で政策会議の専門家と候補入力、#21/#24で通常ニュースと任意の特別報道、#29で終了後の国家史、#30で全期間のAI OFF回帰とサーバー安全策を検証する。AIはfirst playableの依存条件にしない。リポジトリの現行UIは基盤表示段階で、これらのUI接続は未完了である。
